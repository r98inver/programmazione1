...
class ListGraph(object):
	...
	def addEdge(self, edge):
		...
		if not edge.v in self.edges[edge.w]:
			self.edges[edge.w].append(edge.v)
		
		if not edge.w in self.edges[edge.v]:
...
def ShortestPath(G, v, w):
	...
	while len(coda) != 0:
		node, path = coda.pop(0)
		next_nodes = G.neighbourOf(node)
		for n in next_nodes:
			if visto[n] == 1:
				continue
			...
			coda.append([n, path+[node]])
			visto[n] = 1
	
	return 'Cammino non esistente'
...
if __name__ == "__main__":
	...
	if True:
		...
		G = ReadGraph('starwars_small.json')    
